...
#pragma omp parallel for
  for (unsigned n = 0; n < nd_; ++n) {
    ...
    unsigned l = 0;
    // std::sort(nn.pool.begin(), nn.pool.end());
    // if(n==0)std::cout << nn.pool[0].distance<<","<<
    // nn.pool[1].distance<<","<< nn.pool[2].distance<< std::endl;
    while ((l < maxl) && (c < S)) {
      if (nn.pool[l].flag) ++c;
      ++l;
    }
    ...
    for (unsigned l = 0; l < nnhd.M; ++l) {
      ...
      if (nn.flag) {
        ...
        if (nn.distance > nhood_o.pool.back().distance) {
          pthread_mutex_lock(&nhood_o.lock);
          ...
          pthread_mutex_unlock(&nhood_o.lock);
        }
        nn.flag = false;
      } else {
...
#pragma omp parallel for
  for (unsigned i = 0; i < c.size(); i++) {
    ...
    for (unsigned j = 0; j < N; j++) {
      float dist = distance_->compare(data_ + c[i] * dimension_,
                                      data_ + j * dimension_, dimension_);
...
void IndexGraph::eval_recall(
    std::vector<unsigned> &ctrl_points,
...
#pragma omp parallel for
  for (unsigned i = 0; i < nd_; i++) {
    ...
    for (unsigned j = 0; j < S; j++) {
      ...
      if (id == i) continue;
      float dist =
          distance_->compare(data_ + i * dimension_, data_ + id * dimension_,
                             (unsigned)dimension_);
...
void IndexGraph::Build(size_t n, const float *data,
...
void IndexGraph::Search(const float *query, const float *x, size_t K,
                        const Parameters &parameter, unsigned *indices) {
...
void IndexGraph::parallel_graph_insert(unsigned id, Neighbor nn, LockGraph &g,
                                       size_t K) {
  pthread_mutex_lock(&g[id].lock);
  ...
  pthread_mutex_unlock(&g[id].lock);
}

void IndexGraph::GraphAdd(const float *data, unsigned n_new, unsigned dim,
                          const Parameters &parameters) {
  data_ = data;
  data += nd_ * dimension_;
  assert(final_graph_.size() == nd_);
...
#pragma omp for
    for (unsigned i = 0; i < n_new; i++) {
      ...
      get_neighbor_to_add(data + i * dim, parameters, graph_tmp, rng, res,
                          n_new);

      for (unsigned j = 0; j < K; j++) {
        parallel_graph_insert(i + (unsigned)nd_, res[j], graph_tmp, K);
        parallel_graph_insert(
            res[j].id, Neighbor(i + (unsigned)nd_, res[j].distance, true),
            graph_tmp, K);
      }
    }
  };
...
void IndexGraph::get_neighbor_to_add(const float *point,
                                     const Parameters &parameters, LockGraph &g,
                                     std::mt19937 &rng,
  ...
  for (unsigned i = 0; i < L; i++) {
    unsigned id = init_ids[i];
    float dist = distance_->compare(data_ + dimension_ * id, point,
                                    (unsigned)dimension_);
    retset[i] = Neighbor(id, dist, true);
  }
...
void IndexGraph::compact_to_Lockgraph(LockGraph &g) {
  ...
  for (unsigned i = 0; i < final_graph_.size(); i++) {
    ...
    for (unsigned j = 0; j < final_graph_[i].size(); j++) {
      float dist = distance_->compare(data_ + i * dimension_,
                                      data_ + final_graph_[i][j] * dimension_,
                                      (unsigned)dimension_);
